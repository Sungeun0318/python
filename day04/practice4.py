...
# (결과 예시: [1, 9, 25, 49, 81])
b = [i ** 2 for i in range(1,11) if i % 2 == 1]
print(b)
...
# 문제 6: while문과 break (무한 루프) while True: 무한 루프를 사용하여 숫자를 계속 입력받으시오. 사용자가 0을 입력하면 "프로그램 종료"를 출력하며 반복을 멈추고, 
# 그 전까지 입력받은 숫자들의 총합을 출력하시오.
...
for car, minutes in zip(car_numbers, usage_minutes):
    if minutes <= 30:
        fee = 1000
    else:
        a_time = minutes - 30
...
while True:
    ...
    if a == "1":
        b = int(input("입금액을 입력하세요: "))
        balance += b
        print(f"{b}원이 입금되었습니다.")
    elif a == "2":
        ...
        if b > balance:
            ...
            balance -= b
            print(f"{b}원이 출금되었습니다.")
    elif a == "3":
        print(f"현재 잔고는 {balance}원입니다.")
    elif a == "4":
        print("프로그램을 종료합니다.")
        break
    else:
