...
# 제너레이터 : 함수 내부에 yield 키워드를 next()함수를 외부에서 호출하여 yield 키워드까지 실행한다.
def test() :
    print("A 지점 통과")
    yield 1
    print("B 지점 통과")
...
a = next(test)           # (2) 함수 반환값이 저장된 변수를 next에 대입한다.
print(a)                 # (3) yield까지 실행되고 yield 반환값이 반환된다. 

b = next(test)          
print(b)

# c = next(test)           # 다음 yield 없으므로 예외 발생
# print(c)
...
# (1) open 함수 이용하여 지정한 경로의 파일쓰기
file = open('./day06/basic.txt', 'w')  # 현재 .py 파일 폴더내 basic.txt
...
# 스트림이란? 데이터가 흐르는 길, 바이트단위, 프로그래밍언어가 외부 자료와 연결(파일, JDBC, 네트워크 등등)
