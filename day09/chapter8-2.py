...
# [7] 프라이빗 변수, __ 변수명 vs JAVA : private
...
class Circle :
    ...
    # [8] 게터와 세터, 프라이빗 변수을 외부에서 간접 접근 허용 함수
    @property
    def radius(self) :
...
circle = Circle(10)
# print(circle.radius) # 프라이빗 변수이므로 직접 호출시 오류 발생

print(circle.radius) # 간접 접근, getter
circle = 20 # 간접 접근, setter
print(circle.radius) # 간접 접근, getter
...
# [10] 상속 이용한 나만의 예외클래스 만들기
class CustomExeption(Exception) : # Exception 클래스로부터 상속 받는다.
    def __init__(self, message, value):
        super().__init__()
        self.message = message
        self.value = value
        
     # 오버라이드 / 재정의 : 부모의 정의되어 있는 함수를 자식이 다시 정의
    def __str__(self):
        return self.message
    def print(self) : 
        print("### 오류 정보 ###")
