...
# [1] 외부파일 판다스 자료로 불러오기 
# 1. .csv 파일 불러오기
df = pd.read_csv('./day14/data/data.csv',                   # 파일경로
...
# 4. .xml 파일 불러오기 ('pip install xml' 설치 필요)
# xpath = './/가져올태그명', .(현재파일뜻) // (파일전체에서 찾기) row(마크업명)
df = pd.read_xml('./day14/data/data.xml', xpath='.//row')
print(df)
...
# 2. .xlsx 내보내기
df.to_excel('./day14/data/dat_out.xlsx', sheet_name='회원정보')
