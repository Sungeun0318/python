...
with open('./day14/data/T5_data.json', 'r', encoding='utf-8') as json_file :
...
# (1) plt.bar(x축값, y축값)
plt.bar(newDf['연령대'].unique(), newDf.groupby(['연령대']).agg({'고객 수' : 'sum'})['고객 수'], color = 'blue', label = '누적 고객 수')
plt.show()
...
# 차트확인 : 30대 비중이 가장 크고, 50대 고객이 가장 적다
...
# 만약에 겹쳐 나오는 경우 : 아래 순서로 변경할 막대에 bottom = 'df['열이름']
...
# 3. 연령대별(그룹) '평균 구매 금액' 가로막대그래프 / 연령대로 묶어서 mean() 사용
