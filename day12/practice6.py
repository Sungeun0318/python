...
# pandas는 연산을 할 때 짝이 맞지 않는 인덱스가 있으면 에러를 내고, 값이 없다는 뜻으로 nan을 반환한다. 결론은 숫자와 없는 값을 더할 수 없다
...
a = data.sort_index().sort_values(ascending=False, kind='stable') # 1차 정렬에 kind속성에 'stable' 적용하여 유지할 수 있다.
print(a)
...
a = data.groupby(level=0).agg(['sum', 'mean'])
print(a)
